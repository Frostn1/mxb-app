...
### Fixed
- **Creator-locked paints render in the 3D viewer**: a "protected" (locked) paint is
  an encrypted blob, not a plain `PNT\0` file. The paint decoder now transparently
  opens a locked paint (`paint::decode_any`), used everywhere paints are read.
...
- **Rich library cards from inside the `.pkz`**: unlocked (plain-zip) tracks and
  ...
  and downscaled to a small JPEG in Rust. **GUID-locked/encrypted `.pkz` are
