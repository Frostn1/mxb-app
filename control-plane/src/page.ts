/**
 * The page a person sees when Steam sends them back, in mxbsecure.com's look: the wordmark, a
 * bold mono headline and one line of text on a card. No script, nothing to click.
 */
...
/** A branded one-message page. The headline follows the status unless one is given. */
export function page(status: number, message: string, title = status < 400 ? "All set." : "Couldn't sign in."): Response {
  const body =
    ...
    `<style>${STYLE}</style>` +
    `<main><p class="mark">mxbsecure</p><h1>${escape(title)}</h1><p class="msg">${escape(message)}</p></main>`;
