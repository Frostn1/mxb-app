 ...
 * `available` to unlock (has a stored key, and isn't withdrawn, taken down or removed). Lets the
 * app show a locked file with a real name and a reason, without ever needing the content key.
 *
 ...
 *   by its creator. The same conditions `decideEntitlement` refuses a grant on, minus the audit
 *   write: the app polls this on every pass and a row per asset per poll would bury the
 *   creator's real usage log.
...
async function assetStatus(request: Request, account: Account, env: Env): Promise<Response> {
  ...
  const known = await env.DB.prepare(
    `SELECT id, title, wrapped_key IS NOT NULL AS has_key,` +
      ` (withdrawn_at IS NOT NULL OR taken_down_at IS NOT NULL OR deleted_at IS NOT NULL) AS withdrawn` +
  ...
  return json(200, {
    ...
    assets: assetIds.map((id) => {
      ...
      return {
        ...
        // or removed asset counts as revoked for everyone, entitled or not, because the grant
        // refuses it for everyone — the key on disk should stop opening on the same terms. A
        // removal is why the row is kept rather than deleted: an asset we no longer knew would be
        // "can't tell", and the keys for it would stay on every PC that has one.
...
async function decideEntitlement(
  ...
  const decide = async (): Promise<{ allowed: boolean; reason: string; log: boolean }> => {
    ...
    const asset = await env.DB.prepare("SELECT withdrawn_at, taken_down_at, deleted_at FROM assets WHERE id = ?")
      .bind(assetId)
      .first<{ withdrawn_at: number | null; taken_down_at: number | null; deleted_at: number | null }>();
    ...
    // The creator removed it. Its own word rather than "withdrawn", because this one is final:
    // the content key is gone, so there is nothing left to release even if it were allowed.
    if (asset.deleted_at !== null) return { allowed: false, reason: "removed", log: true };
