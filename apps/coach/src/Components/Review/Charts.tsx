...
function scale(vs: number[], h: number, lo?: number, hi?: number) {
  ...
  return { min, max, y: (v: number) => h - 6 - ((v - min) / span) * (h - 22) };
}

function Row({ label, hint, height, right, children }: { label: string; hint?: string; height: number; right?: ReactNode; children: ReactNode }) {
  return (
    <div className="relative border-t border-border first:border-t-0">
      <div className="pointer-events-none absolute left-3 top-1.5 z-10 flex items-baseline gap-2 text-[11px]">
        <span className="font-semibold">{label}</span>
        {hint && <span className="text-faint">{hint}</span>}
      </div>
      {right && <div className="pointer-events-none absolute right-3 top-1.5 z-10 font-mono text-[11px] tabular-nums text-muted-foreground">{right}</div>}
      <svg viewBox={`0 0 ${W} ${height}`} preserveAspectRatio="none" style={{ height }} className="block w-full">
        {children}
      </svg>
    </div>
  );
...
export default function Charts({
  ...
  const markers = zoomed ? sel.findings.map((f, k) => ({ n: k + 1, at: f.at })).filter((m) => m.at >= a && m.at <= b) : [];
  ...
  const lane = (throttle: number[], brake: number[], y0: number, h: number) => {
    ...
    return (
      <>
        <rect x={0} y={y0} width={W} height={h} fill="var(--foreground)" fillOpacity={0.03} />
    ...
    <div>
      ...
      <div className="relative select-none" onMouseMove={move} onMouseLeave={() => onCursor(null)}>
        <Row
          ...
          height={150}
          right={at != null ? `${ch.speed.lap[at].toFixed(0)} / ${ch.speed.reference[at].toFixed(0)} km/h` : `${sp.max.toFixed(0)} km/h`}
        >
          {bands(xs, you, fast, sp.y).map((band, k) => (
            <polygon key={k} points={band.points} fill={band.up ? "var(--success)" : "var(--destructive)"} fillOpacity={0.22} />
          ))}
        ...
        <Row label={t("review.inputs")} hint={t("review.inputsHint")} height={78}>
          {lane(pick(ch.throttle.lap), pick(ch.brake.lap), 20, 24)}
          {lane(pick(ch.throttle.reference), pick(ch.brake.reference), 50, 24)}
        </Row>
        <div className="pointer-events-none absolute left-3 z-10 text-[10px] font-semibold text-muted-foreground" style={{ top: 150 + 22 }}>
          {t("review.you")}
        </div>
        <div className="pointer-events-none absolute left-3 z-10 text-[10px] font-semibold text-muted-foreground" style={{ top: 150 + 52 }}>
          {t("review.fastLap")}
        </div>
        ...
        <Row label={t("review.lean")} height={70} right={at != null ? `${Math.abs(ch.lean.lap[at]).toFixed(0)}° / ${Math.abs(ch.lean.reference[at]).toFixed(0)}°` : undefined}>
          {(() => {
            const l = scale([...pick(ch.lean.lap), ...pick(ch.lean.reference)].map(Math.abs), 70, 0);
            return (
              <>
                <polyline points={poly(xs, pick(ch.lean.reference).map(Math.abs), l.y)} stroke="var(--faint)" strokeWidth={1.5} {...stroke} />
                <polyline points={poly(xs, pick(ch.lean.lap).map(Math.abs), l.y)} stroke="var(--primary)" strokeWidth={2} {...stroke} />
              </>
            );
          })()}
        </Row>
        ...
        {hasTravel && (
          <Row label={t("review.travel")} hint={t("review.travelHint")} height={70} right={at != null ? `${ch.fork.lap[at].toFixed(0)}% / ${ch.shock.lap[at].toFixed(0)}%` : undefined}>
            {(() => {
              const s = scale([0], 70, 0, 110);
              return (
                <>
                  <line x1={0} x2={W} y1={s.y(95)} y2={s.y(95)} stroke="var(--destructive)" strokeOpacity={0.6} strokeDasharray="6 5" vectorEffect="non-scaling-stroke" />
                  <polyline points={poly(xs, pick(ch.shock.lap), s.y)} stroke="var(--warning)" strokeWidth={1.5} {...stroke} />
                  <polyline points={poly(xs, pick(ch.fork.lap), s.y)} stroke="var(--primary)" strokeWidth={1.5} {...stroke} />
                </>
              );
            })()}
          </Row>
        ...
        <div className="pointer-events-none absolute inset-0">
          {markers.map((m) => (
            <div key={m.n} className="absolute bottom-0 top-0 border-l border-dashed border-foreground/50" style={{ left: `${pct(m.at)}%` }}>
              <span className="absolute -left-2 top-7 flex size-4 items-center justify-center rounded-full bg-foreground text-[10px] font-bold text-background">
                {m.n}
              </span>
            </div>
          ))}
          {cursor != null && cursor >= a && cursor <= b && (
            <div className="absolute bottom-0 top-0 border-l border-foreground/70" style={{ left: `${pct(cursor)}%` }} />
          )}
        </div>
      </div>
