...
/// attaching to `Frost Mod Manager.exe` once it is up — is caught too. In debug builds it does nothing.
pub fn guard() {
    #[cfg(not(debug_assertions))]
    {
        ...
        // pick `Frost Mod Manager.exe`), so a one-shot check at startup would miss the very attack it
