//! The in-game overlay both apps show: a frameless, always-on-top window summoned by a global
//! hotkey, so the player can act without alt-tabbing out of the game.
//!
//! Each app loads its own bundle with `?overlay=1` into a window labelled [`LABEL`]. When MXB
//! App and MXB Coach both run, [`crate::overlaylink`] joins them: MXB App holds the one hotkey,
//! each tab bar lists the other's tabs, and a click there hands the screen across.
//!
...
use crate::overlaylink::{App, Event, Link, Msg, PeerInfo, Rect};

/// Window label, in both apps. Also the label each overlay capability grants permissions to.
...
/// To the overlay: switch to this tab (a string id).
pub const TAB_EVENT: &str = "overlay-tab";
/// To every window: the linked app changed (`PeerInfo` or null).
pub const PEER_EVENT: &str = "overlay-peer";
...
/// What one app's overlay window is.
pub struct Spec {
    pub app: App,
    /// The frontend entry: `index.html?overlay=1`. `main.tsx` branches on the flag.
    pub url: &'static str,
    pub title: &'static str,
    pub size: (f64, f64),
    pub min_size: (f64, f64),
    /// Tab ids, offered to the other app's tab bar.
...
/// The linked app's overlay is on screen (as far as its messages say).
static PEER_VISIBLE: AtomicBool = AtomicBool::new(false);
/// The overlay last put on screen was the linked app's, so the hotkey reopens that one.
static LAST_WAS_PEER: AtomicBool = AtomicBool::new(false);
...
/// startup only logs the failure, so without this the player's evidence is "I pressed it
/// and nothing happened". Settings reads it and names the cause.
static HOTKEY_ERROR: Mutex<Option<String>> = Mutex::new(None);

pub fn record_hotkey_result(result: &Result<(), String>) {
...
/// Point the configured combo at this app's overlay. The caller unregisters first.
pub fn bind_toggle<R: Runtime>(
    app: &AppHandle<R>,
    spec: &'static Spec,
    ...
    app.global_shortcut()
        ...
        .map_err(|e| format!("Couldn't register {combo} — another app is probably using it. ({e})"))?;
...
/// The link, once the app has started one.
pub fn link<R: Runtime>(app: &AppHandle<R>) -> Option<Link> {
    app.try_state::<Link>().map(|l| l.inner().clone())
}

/// The linked app, if there is one.
pub fn peer<R: Runtime>(app: &AppHandle<R>) -> Option<PeerInfo> {
    link(app)?.peer()
}
...
/// hotkey shouldn't pay for a second webview. `tab` rides in the URL, since a new webview
/// can't hear an event yet.
fn ensure_window<R: Runtime>(
    ...
    let url = match tab {
        Some(tab) => format!("{}&tab={tab}", spec.url),
...
/// heuristic, and a wrong guess must not be what stops the overlay from opening.
...
fn is_up<R: Runtime>(app: &AppHandle<R>) -> bool {
    app.get_webview_window(LABEL)
        .is_some_and(|w| w.is_visible().unwrap_or(false))
}

/// Put this app's overlay on screen: on `tab` if given, centred on `near` (the other app's
/// overlay, on a handoff) or else over the game.
pub fn show<R: Runtime>(
    ...
    let window = ensure_window(app, spec, tab.as_deref())?;
    if gamewindow::is_exclusive_fullscreen() {
        fullscreen_hint(app);
    }
    match near {
        Some(rect) => place_on(&window, rect),
        None => center_over_game(&window),
    }
    if let (Some(tab), false) = (&tab, fresh) {
        let _ = app.emit_to(LABEL, TAB_EVENT, tab.clone());
    }
    window.show().map_err(|e| format!("{e:#}"))?;
    SHOWINGS.fetch_add(1, Ordering::SeqCst);
    LAST_WAS_PEER.store(false, Ordering::SeqCst);
...
/// The hotkey: close whichever overlay is up, or open the one used last.
pub fn toggle<R: Runtime>(app: &AppHandle<R>, spec: &Spec) -> Result<(), String> {
    if is_up(app) {
        return hide(app);
    }
    if let Some(link) = link(app) {
        if let Some(peer) = link.peer() {
            if PEER_VISIBLE.load(Ordering::SeqCst) {
                // Ours to give: the key press was ours, so the other app may take the
                // foreground to hand it back to the game.
...
fn ask_peer_to_show(link: &Link, peer: &PeerInfo, tab: Option<String>, rect: Option<Rect>) {
    // Windows only lets a process take the foreground if the last input was its own.
    gamewindow::allow_foreground(peer.pid);
    if link.send(&Msg::Show { tab, rect }) {
        PEER_VISIBLE.store(true, Ordering::SeqCst);
        LAST_WAS_PEER.store(true, Ordering::SeqCst);
    }
}

/// A click on one of the other app's tabs: put ours away and have it show its overlay
/// where ours was, on that tab.
pub fn handoff<R: Runtime>(app: &AppHandle<R>, tab: String) -> Result<(), String> {
    let link = link(app).ok_or("The other app isn't linked.")?;
    let peer = link.peer().ok_or("The other app isn't running.")?;
    let rect = app.get_webview_window(LABEL).and_then(|w| {
        let (p, s) = (w.outer_position().ok()?, w.outer_size().ok()?);
        Some((p.x, p.y, p.x + s.width as i32, p.y + s.height as i32))
    });
    // `dismiss`, not `hide`: the other app is taking the screen, not the game.
    dismiss(app)?;
    ask_peer_to_show(&link, &peer, Some(tab), rect);
    Ok(())
}

/// Everything the link says that means the same in both apps. `Rebind` is the caller's.
pub fn on_link_event<R: Runtime>(app: &AppHandle<R>, spec: &Spec, event: &Event) {
    match event {
        Event::Up(peer) => {
            let _ = app.emit(PEER_EVENT, Some(peer.clone()));
        }
        Event::Down => {
            PEER_VISIBLE.store(false, Ordering::SeqCst);
            LAST_WAS_PEER.store(false, Ordering::SeqCst);
            let _ = app.emit(PEER_EVENT, None::<PeerInfo>);
        }
        Event::Msg(Msg::Show { tab, rect }) => {
            if let Err(e) = show(app, spec, tab.clone(), *rect) {
                log::warn!("overlay didn't show for the other app: {e}");
                if let Some(link) = link(app) {
                    link.send(&Msg::Hidden { reason: "failed".into() });
                }
            }
        }
        Event::Msg(Msg::Hide) => {
        ...
        Event::Msg(Msg::Hidden { .. }) => PEER_VISIBLE.store(false, Ordering::SeqCst),
        Event::Msg(Msg::Shown) => {
            PEER_VISIBLE.store(true, Ordering::SeqCst);
            LAST_WAS_PEER.store(true, Ordering::SeqCst);
...
///
/// Deliberately not fired on every blur: our own file picker takes focus off the overlay
/// too, and closing the window out from under a dialog it opened is worse than the bug.
/// Hence the wait, and the check for *whose* window took over.
pub fn on_focus_lost<R: Runtime>(app: &AppHandle<R>) {
    ...
    std::thread::spawn(move || {
        ...
        let blur = Blur {
            ...
            foreground_is_another_app: gamewindow::foreground_is_another_app(),
        };
...
fn center_over_game<R: Runtime>(window: &WebviewWindow<R>) {
    if let Some(rect) = gamewindow::game_window_rect() {
        place_on(window, rect);
    }
}

fn place_on<R: Runtime>(window: &WebviewWindow<R>, rect: Rect) {
...
///
/// The clamp is the belt to [`gamewindow::game_window_rect`]'s braces: no rect from
/// anywhere can put the overlay somewhere the player can't see it — an overlay off the
/// edge of the desktop is indistinguishable from a hotkey that did nothing.
...
/// The monitor `rect` sits on, as `left, top, right, bottom`, found by the rect's centre:
/// the point is to move the overlay to the screen the *game* is on.
...
///
/// Separate from [`hide`] for "Open full app" and handoffs: focus is on its way somewhere
/// else, so pulling MX Bikes forward on the way out would only fight it.
pub fn dismiss<R: Runtime>(app: &AppHandle<R>) -> Result<(), String> {
    if let Some(window) = app.get_webview_window(LABEL) {
        let was_up = window.is_visible().unwrap_or(false);
        window.hide().map_err(|e| format!("{e:#}"))?;
        if was_up {
            if let Some(link) = link(app) {
                link.send(&Msg::Hidden { reason: "closed".into() });
            }
        }
    }
    Ok(())
...
mod tests {
    ...
    const OVERLAY: (u32, u32) = (SPEC.size.0 as u32, SPEC.size.1 as u32);
    ...
    /// A minimized game reports the iconic rect, and an overlay centred on it is placed
    /// ~32,000px off the left of the desktop. `game_window_rect` refuses that rect; this
    /// clamp holds for *any* rect we haven't thought of.
    #[test]
    ...
    /// A screen to the left of primary has negative coordinates, and a clamp that assumed
    /// otherwise would drag the overlay back onto the wrong display.
    #[test]
    ...
    /// A handoff centres the other app's overlay on this one's rect, whatever their sizes.
    #[test]
    fn a_handed_off_overlay_is_centred_where_the_other_one_was() {
        let coach = (660, 200, 1220, 880); // 560x680
        assert_eq!(centered_on(coach, OVERLAY, Some(PRIMARY)), (390, 180));
    }
    ...
    fn summoning_twice_reuses_the_one_window() {
        let app = mock_app();
        let first = ensure_window(app.handle(), &SPEC, None).expect("the overlay window is creatable");
        let second = ensure_window(app.handle(), &SPEC, Some("locker")).expect("second call reuses the window");
    ...
    fn toggling_never_builds_a_second_overlay() {
        let app = mock_app();
        toggle(app.handle(), &SPEC).expect("first toggle");
        toggle(app.handle(), &SPEC).expect("second toggle");
        toggle(app.handle(), &SPEC).expect("third toggle");
    ...
    /// No link started: a handoff has nowhere to go, and says so rather than hiding the
    /// overlay for nothing.
    #[test]
    fn a_handoff_without_a_link_is_an_error() {
        let app = mock_app();
        assert!(handoff(app.handle(), "tips".into()).is_err());
    }
    ...
    /// overlay exactly like a click into the game does.
    #[test]
